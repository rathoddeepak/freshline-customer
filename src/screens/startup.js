	...
	render() {
		...
		return (
			<KeyboardAvoidingView style={s.inptr} enabled={false}>				
			    <Text style={s.tt}>Fresh Line</Text>
				<Inputer toHome={this.landHome} onBusy={busy => this.setState({busy})} />				
				<Animated.View pointerEvents="none" style={[{opacity}, s.btn]}>
					<LottieView		        
				        autoPlay
				        loop
				        style={{width:'100%',height:450,alignSelf:'center',top:20,left:'-1.4%'}}
				        source={require('assets/anims/cute_chicken.json')}
				    />			    
			    </Animated.View>			    			    
...
class Inputer extends Component {
	...
	render() {
		...
		return (
			<ViewPager initialPage={0} ref={ref => this.pager = ref} style={{width:'100%',alignSelf:'center',height:260}} scrollEnabled={false}>
				<View style={s.deck}>
					 <Text style={s.dd}>We will send verifcation code to this number...</Text>
					 <View style={s.inh}>
				...
				<View style={s.deck}>
					 <Text style={[s.dd, {marginBottom:0}]}>We have sent OTP on your registered mobile number...</Text>
					 <OTPInputView
					    style={s.otpv}
					    pinCount={6}
				 ...
				 <View style={s.deck}>
				 	<Text style={s.dd}>Please fill your good name...!</Text>
				 	<TextInput style={s.inBox} placeholder="Full Name" placeholderTextColor={helper.secondaryColor} value={name} onChangeText={name => this.setState({name})}/>
		...
		return (
			<View style={{width:'100%'}}>
			 ...
			 :
				...
				<Text style={[s.dtf, {width:115}]} onPress={this.change}> | Change Number</Text>
				</View>
			 }
			</View>
		)
...
const s = StyleSheet.create({
	deck:{paddingLeft:'2.5%'},
	inptr:{height:'100%',width:'100%',backgroundColor:helper.primaryColor,justifyContent:'center',alignItems:'center'},
	inh:{flexDirection:'row',alignItems:'center',marginBottom:20},
	ctTxt:{fontSize:30,width:70,color:helper.white,fontWeight:'bold'},
	header:{height:200,width:'100%',marginBottom:30,marginTop:20},
	dtf:{fontSize:13,color:helper.secondaryColor,fontWeight:'bold',marginBottom:10},
	...
	tt:{width:'97%',fontSize:55,color:helper.white,fontWeight:'bold'},
	dd:{fontSize:15,color:helper.secondaryColor,marginBottom:15},
	ee:{height:45,width:'95%',justifyContent:'center',alignItems:'center',backgroundColor:helper.white,elevation:24,borderRadius:10},
	inpt:{fontSize:30,fontWeight:'bold',color:helper.white,padding:0,width:'100%'},
	btn:{width:'100%',bottom:0,position:'absolute'},
	otpv:{width:'93%',alignSelf:'center',height:40,marginBottom:20},
	bs:{width: 30,height: 45,borderWidth: 0,borderBottomWidth: 1, color:helper.white},
	z:{borderColor: helper.white},
	inBox:{borderWidth:2,borderRadius:10,borderColor:helper.secondaryColor,color:helper.white,fontSize:20,marginBottom:20,paddingLeft:10},
